...
const jsonDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2021/data/json'));
const htmlDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2021/html'));
 ...
 * @param {string} dateStr - Date string like "February 15, 2021"
 * @returns {string} - YYYYMMDD format like "20210215"
 */
...
function applyMissingInfoLogic(extractedData) {
    ...
    // Apply specific award fixes from 2021logic.txt FIRST (before general logic)
    extractedData = applySpecific2021Fixes(extractedData);
...
/**
 * Apply specific 2021 award fixes based on 2021logic.txt and orchidNameDecisionTree.json
 * 
 * UPDATED 2021 FIXES REFERENCE:
 * - Award 20215262: Missing Date (Oct 4, 2021), Location (San Francisco Monthly), SourceURL
 * - Award 20215263: Missing Date/Location - Check Source HTML
 * - Award 20215304: Missing Award - Award is "AD" (Award of Distinction), awardpoints N/A
 * - Award 20215347: Missing Date/Location - Check Source HTML
 * - Award 20215348: Missing Date/Location - Check Source HTML  
 * - Award 20215349: Missing Date/Location - Check Source HTML
 * - Award 20215350: Missing Date/Location - Check Source HTML
 * - Award 20215415: Display SC (Silver Certificate), genus/species/cross/clone N/A, isDisplay true
 * - Award 20215306: Cross is "Cymbidium Shirley May Walker x Cymbidium Bill Bailey"
 * - Award 20215308: Apply orchidNameDecisionTree.json rules for plant naming
 * - Award 20215329: Apply orchidNameDecisionTree.json rules for plant naming
 * - Award 20215334, 20215335: Apply orchidNameDecisionTree.json rules for plant naming
 ...
 */
function applySpecific2021Fixes(extractedData) {
    const awardNum = extractedData.awardNum;
    
    // Award 20215262 - Missing Date/Location fix
    if (awardNum === '20215262') {
        extractedData.date = 'October 4, 2021';
    ...
    // Award 20215263 - Missing Date/Location (extract from source HTML)
    if (awardNum === '20215263') {
    ...
    // Award 20215304 - Missing Award Info fix
    if (awardNum === '20215304') {
        extractedData.award = 'AD'; // Award of Distinction
        extractedData.awardpoints = 'N/A'; // No point value for AD
    }
    
    // Awards 20215347, 20215348, 20215349, 20215350 - Missing Date/Location (extract from source HTML)
    if (['20215347', '20215348', '20215349', '20215350'].includes(awardNum)) {
        const dateLocation = extractDateLocationFromHtml(awardNum);
        if (dateLocation) {
            extractedData.date = dateLocation.date;
            extractedData.location = dateLocation.location;
    ...
    // Award 20215415 - Display SC (Silver Certificate) fix
    if (awardNum === '20215415') {
    ...
    // Award 20215306 - Cross fix
    if (awardNum === '20215306') {
        extractedData.cross = 'Cymbidium Shirley May Walker x Cymbidium Bill Bailey';
    }
    
    // Award 20215308 - Apply orchidNameDecisionTree.json rules
    if (awardNum === '20215308') {
        // Based on 2021logic.txt: Phalaenopsis Matthew Berry 'My Love' (Phalaenopsis schilleriana x Phalaenopsis leucorrhoda)
    ...
    // Award 20215329 - Apply orchidNameDecisionTree.json rules  
    if (awardNum === '20215329') {
        // Based on 2021logic.txt: Paphiopedilum Wossner Black Wings 'Memoria Scott Collins' (Paphiopedilum rothschildianum 'Leo' x Paphiopedilum adductum 'Ace')
    ...
    // Awards 20215334, 20215335 - Apply orchidNameDecisionTree.json rules
    if (awardNum === '20215334') {
    ...
    // Award 20215336 - Add missing cross (extracted from HTML)
    if (awardNum === '20215336') {
        extractedData.cross = 'Vanda critata x Vanda tricolor var. suavis';
    }
    
    // Award 20215348 - Add missing cross (extracted from HTML)
    if (awardNum === '20215348') {
        extractedData.cross = 'Rhyncholaeliocattleya Waikiki Gold x Rhyncholaeliocattleya Vicky Nazareno';
    }
    
    // Award 20215349 - Add missing cross (extracted from HTML) 
    if (awardNum === '20215349') {
        extractedData.cross = 'Cattlianthe Blue Boy x Guarianthe bowringiana';
    }
    
    // Award 20215365 - Silver Certificate display award fix (existing)
    if (awardNum === '20215365') {
    ...
    // Awards 20215256, 20215258 - Dendrobium cross fix (existing)
    if (awardNum === '20215256' || awardNum === '20215258') {
        extractedData.cross = "'Royal Blue' AM/AOS x 'Blues Brothers'";
    }
    
    // Award 20215297 - Cattleya x blossfeldiana fix (existing)
    if (awardNum === '20215297') {
        extractedData.genus = 'Cattleya';
    ...
    // Award 20215301 - Cattleya warscewiczii cross fix (existing)
    if (awardNum === '20215301') {
        extractedData.cross = "'La Florista' x 'Anita'";
    }
    
    // Award 20215358 - Paphiopedilum measurements "NM" fix (existing)
    if (awardNum === '20215358') {
        if (extractedData.measurements) {
    ...
    // Apply display award logic for all display awards per 2021logic.txt
    ...
    if (isDisplayAward) {
        ...
        // Set plant fields per 2021logic.txt: "Put N/A for any species not specified"
    ...
    try {
        ...
        if (mainFont.length > 0) {
            ...
            if (lines.length > 0) {
                ...
                // Format 1: "October 4, 2021 San Francisco Monthly"
                ...
                if (firstLine.includes(' - ')) {
                    ...
                    // Look for year (2021) or month patterns
                    ...
                    if (monthDayMatch) {
                        return {
                            date: monthDayMatch[1].trim() + ', 2021', // Add year since we know it's 2021
...
function extractFullAwardDataFromHtml(awardNum) {
    ...
    try {
        ...
        const extractedData = {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2021/html/${awardNum}.html`,
            year: 2021
        };
...
function processAll2021Files() {
        console.log('🚀 PROCESSING ALL 2021 HTML FILES TO JSON (Enhanced with Logic Reference)');
    ...
    // Get all HTML files, excluding summary pages (20210xxx are summary pages, 20215xxx are individual awards)
    const htmlFiles = fs.readdirSync(htmlDirectory)
        .filter(file => file.endsWith('.html') && file !== '2021.html')
        .filter(file => !file.match(/^20210/)) // Skip summary pages like 20210120.html
    ...
    const reportPath = path.join(jsonDirectory, '../2021-enhanced-processing-report.json');
    const report = {
        timestamp: new Date().toISOString(),
 ...
 * @param {string} year - The year for the analysis (default: 2021)
 */
function saveAnalysisToFile(analysis, focusedMode, year = '2021') {
 ...
 */
async function analyze2021Data(focusedMode = true, saveToFile = true) {
    console.log(focusedMode ? '🔍 2021 AWARDS - MISSING DATA ANALYSIS' : '📊 2021 AWARDS - FULL DATA ANALYSIS');
    console.log('='.repeat(80));
    ...
    try {
        ...
        if (saveToFile) {
            saveAnalysisToFile(analysis, focusedMode, '2021');
        }
