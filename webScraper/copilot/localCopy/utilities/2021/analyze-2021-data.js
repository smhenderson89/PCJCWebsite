...
const { analyze2021Data } = require('./2021htmlToJSONparse.js');
 ...
 */
async function analyze2021DataWithSourceUrls(focusedMode = true, saveFile = true, includeSourceUrls = true) {
    console.log(`🔍 Running enhanced analysis with source URL inclusion for review...`);
    
    // Get the base analysis results
    const results = await analyze2021Data(focusedMode, false); // Don't save yet, we'll enhance first
    ...
    if (saveFile) {
        ...
        const filename = `2021-${mode}-${timestamp}.json`;
        ...
        const enhancedOutput = {
            metadata: {
                ...
                generatedBy: 'analyze-2021-data.js (enhanced with source URLs)',
...
/**
 * Standalone 2021 Analysis Runner
 * 
 * This file provides a simple way to run 2021 data analysis without command line arguments.
 * Just run: node analyze-2021-data.js
 * 
 * Features:
...
async function runAnalysisMenu() {
    console.log('🔍 2021 ORCHID AWARDS DATA ANALYSIS TOOL');
    ...
    try {
        const results = await analyze2021DataWithSourceUrls(true, true, true); // focusedMode=true, saveToFile=true, includeSourceUrls=true
...
async function runSpecificAnalysis(type = 'focused', saveFile = true) {
    console.log(`🔍 Running ${type} analysis for 2021 data...`);
    
    const focusedMode = type !== 'full';
    
    try {
        const results = await analyze2021Data(focusedMode, saveFile);
        return results;
    } catch (error) {
...
async function quickMissingDataAnalysis() {
    console.log('🎯 QUICK MISSING DATA ANALYSIS - 2021 (with source URLs for review)');
    console.log('='.repeat(60));
    
    try {
        const results = await analyze2021DataWithSourceUrls(true, true, true);
...
async function main() {
    ...
    if (args.length === 0) {
        ...
        await analyze2021Data(true, true);
    } else {
        console.log('❓ Unknown option:', args[0]);
        console.log('');
        console.log('Available options:');
        console.log('  node analyze-2021-data.js          # Default menu');
        console.log('  node analyze-2021-data.js --quick   # Quick summary');
        console.log('  node analyze-2021-data.js --full    # Full analysis');
        console.log('  node analyze-2021-data.js --focused # Focused (missing data only)');
        console.log('  node analyze-2021-data.js --no-save # No file output');
        console.log('  node analyze-2021-data.js --no-urls  # Faster analysis without source URLs');
    }
}
