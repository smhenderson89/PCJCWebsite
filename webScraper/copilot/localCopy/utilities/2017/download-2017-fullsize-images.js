...
console.log('📸 Starting 2017 Full-Size Images Download Process...\n');
...
function mapAwardToDirectory() {
    ...
    // Read the 2017-index.html to map awards to their directories
    const indexPath = path.join(HTML_DIR, '2017-index.html');
    if (fs.existsSync(indexPath)) {
        const indexContent = fs.readFileSync(indexPath, 'utf8');
        
        // Look for patterns like: <A HREF="20170103/20174115.html">
        const linkRegex = /<A HREF="([^"]+\/(20174\d{3})\.html)"/gi;
        let match;
...
function extractAwardImages() {
    ...
    // Get all award HTML files (20174XXX.html pattern)
    const awardFiles = fs.readdirSync(HTML_DIR)
        .filter(file => file.match(/^20174\d{3}\.html$/));
    ...
    for (const awardFile of awardFiles) {
        ...
        // Look for image references like: <IMG SRC="20174115.jpg"
...
async function downloadFullSizeImages() {
    try {
        ...
        if (downloadCount > 0 || skippedCount > 0) {
            ...
            console.log(`\n✅ 2017 full-size image download complete!`);
        } else {
