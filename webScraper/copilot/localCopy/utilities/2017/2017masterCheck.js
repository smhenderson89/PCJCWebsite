/**
 * 2017 Master Data Check Script
 * Comprehensive verification of 2017 orchid awards data
...
// Path to the 2017 JSON data directory
const jsonDataPath = path.join(__dirname, '../../paccentraljc.org/awards/2017/data/json');
...
function analyzeMeasurementType(data) {
    ...
    result.isDisplay = data.genus && (data.genus.toLowerCase() === "display" || 
...
function checkMissingData(data) {
    ...
    const isDisplay = data.genus && (data.genus.toLowerCase() === "display" || 
                                    data.genus.toLowerCase().includes("display"));
...
function runMasterCheck() {
    console.log('🔍 2017 MASTER DATA CHECK');
    ...
    const reportPath = path.join(__dirname, '2017-master-check-report.json');
...
function printSummary(results) {
    console.log('\n' + '=' .repeat(80));
    console.log('📊 2017 DATA VERIFICATION SUMMARY');
    ...
    if (results.filesWithIssues === 0) {
        console.log('🎉 ALL DATA CHECKS PASSED! The 2017 dataset is clean and properly categorized.');
    } else {
