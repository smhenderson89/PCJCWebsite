...
const INDEX_URL = 'https://www.paccentraljc.org/2017.html';
const BASE_DIR = path.join(__dirname, '../../paccentraljc.org/awards/2017');
const HTML_DIR = path.join(BASE_DIR, 'html');
const DATA_DIR = path.join(BASE_DIR, 'data');

console.log('🌐 Starting 2019 Awards Download Process...\n');
...
function extractAwardLinks(htmlContent) {
    ...
    // Look for links that match 2017 award patterns
    ...
    while ((match = linkRegex.exec(htmlContent)) !== null) {
        ...
        // Match 2017 award number patterns (20174XXX)
        if (href.match(/20174\d{3}\.html$/)) {
            links.push({
                href: href,
...
async function downloadAwards() {
    try {
        ...
        console.log('📄 Step 1: Downloading 2017 index page...');
        const indexPath = path.join(HTML_DIR, '2017-index.html');
        
        const indexContent = await downloadFile(INDEX_URL, indexPath);
        console.log(`✅ Downloaded: 2017-index.html`);
        ...
        if (awardLinks.length === 0) {
            ...
            const allLinks = [];
            const altRegex = /href=["']([^"']*(?:2017|award)[^"']*)["']/gi;
            let altMatch;
            ...
            console.log(`\n🔍 Alternative parsing found ${allLinks.length} links with "2017" or "award":`);
        ...
        if (downloadCount > 0) {
            ...
            console.log(`\n✅ 2017 download complete! You can now run the analysis script.`);
        } else {
