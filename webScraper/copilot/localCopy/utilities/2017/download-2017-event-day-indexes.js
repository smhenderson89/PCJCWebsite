...
console.log('📅 Starting 2017 Event Day Index Pages Download Process...\n');
...
// Extract event day index page links from main 2017 index
function extractEventDayIndexLinks() {
    const indexPath = path.join(HTML_DIR, '2017-index.html');
    
    if (!fs.existsSync(indexPath)) {
        throw new Error('2017-index.html not found. Please run the main download script first.');
    }
    
    const indexContent = fs.readFileSync(indexPath, 'utf8');
    const eventDayLinks = [];
    
    // Look for patterns like: <A HREF="20170103/20170103.html">
    // This matches the YYYYMMDD/YYYYMMDD.html pattern for event day index pages
    const linkRegex = /<A HREF="(2017\d{4})\/\1\.html"[^>]*>/gi;
    let match;
...
async function downloadEventDayIndexPages() {
    try {
        console.log('🔍 Step 1: Extracting event day index page links from 2017-index.html...');
        ...
        if (downloadCount > 0 || skippedCount > 0) {
            ...
            console.log(`\n✅ 2017 event day index pages download complete!`);
        } else {
