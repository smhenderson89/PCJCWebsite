/**
 * 2019 Extract Missing Images from Local HTML Files
...
// Paths
const reportPath = path.join(__dirname, 'reports/2019-image-check-report.json');
const jsonDataPath = path.join(__dirname, '../../paccentraljc.org/awards/2019/data/json');
const htmlDataPath = path.join(__dirname, '../../paccentraljc.org/awards/2019/html');
...
async function extractMissingImages() {
    console.log('📸 2019 EXTRACT MISSING IMAGES FROM LOCAL HTML');
    console.log('=' .repeat(60));
    
    // Read the image check report
    if (!fs.existsSync(reportPath)) {
        console.error(`❌ Report not found: ${reportPath}`);
        console.log('Please run 2019-check-images.js first to generate the report.');
        return;
    }
    ...
    // Print summary
    console.log('\\n' + '=' .repeat(80));
    console.log('📊 2019 MISSING IMAGE EXTRACTION SUMMARY');
    ...
    // Save results
    const extractionReportPath = path.join(__dirname, 'reports/2019-image-extraction-report.json');
