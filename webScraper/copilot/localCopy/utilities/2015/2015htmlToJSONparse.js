...
const jsonDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2015/data/json'));
const htmlDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2015/html'));
 ...
 * @param {string} dateStr - Date string like "February 15, 2015"
 * @returns {string} - YYYYMMDD format like "20150215"
 */
...
function applyMissingInfoLogic(extractedData) {
    ...
    // Apply specific award fixes from 2015logic.txt FIRST (before general logic)
    extractedData = applySpecific2015Fixes(extractedData);
...
/**
 * Apply specific 2015 award fixes based on 2015logic.txt and orchidNameDecisionTree.json
 * 
 ...
 */
function applySpecific2015Fixes(extractedData) {
    const awardNum = extractedData.awardNum;
    
    // Apply display award logic for all display awards per 2015logic.txt
    ...
    if (isDisplayAward) {
        ...
        // Set plant fields per 2015logic.txt: "Put N/A for any species not specified"
    ...
    try {
        ...
        if (mainFont.length > 0) {
            ...
            if (lines.length > 0) {
                ...
                // Format 1: "October 4, 2015 San Francisco Monthly"
                ...
                if (firstLine.includes(' - ')) {
                    ...
                    // Look for year (2015) or month patterns
                    ...
                    if (monthDayMatch) {
                        return {
                            date: monthDayMatch[1].trim() + ', 2015', // Add year since we know it's 2015
...
function extractFullAwardDataFromHtml(awardNum) {
    ...
    try {
        ...
        const extractedData = {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2015/html/${awardNum}.html`,
            year: 2015
        };
...
function processAll2015Files() {
        console.log('🚀 PROCESSING ALL 2015 HTML FILES TO JSON (Enhanced with Logic Reference)');
    ...
    // Get all HTML files, excluding summary pages (20150xxx are summary pages, 20155xxx are individual awards)
    const htmlFiles = fs.readdirSync(htmlDirectory)
        .filter(file => file.endsWith('.html') && file !== '2015.html')
        .filter(file => !file.match(/^20150/)) // Skip summary pages like 20150120.html
    ...
    const reportPath = path.join(jsonDirectory, '../2015-enhanced-processing-report.json');
    const report = {
        timestamp: new Date().toISOString(),
 ...
 * @param {string} year - The year for the analysis (default: 2015)
 */
function saveAnalysisToFile(analysis, focusedMode, year = '2015') {
 ...
 */
async function analyze2015Data(focusedMode = true, saveToFile = true) {
    console.log(focusedMode ? '🔍 2015 AWARDS - MISSING DATA ANALYSIS' : '📊 2015 AWARDS - FULL DATA ANALYSIS');
    console.log('='.repeat(80));
    ...
    try {
        ...
        if (saveToFile) {
            saveAnalysisToFile(analysis, focusedMode, '2015');
        }
