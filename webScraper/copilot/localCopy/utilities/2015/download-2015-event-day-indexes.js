...
console.log('📅 Starting 2015 Event Day Index Pages Download Process...\n');
...
function extractEventDayIndexLinks() {
    ...
    if (!fs.existsSync(indexPath)) {
        throw new Error('2015-index.html not found. Please run the main download script first.');
    }
    
    const indexContent = fs.readFileSync(indexPath, 'utf8');
    const eventDayLinks = [];
    
    // Look for patterns like: <A HREF="20150103/20150103.html">
    // This matches the YYYYMMDD/YYYYMMDD.html pattern for event day index pages
    const linkRegex = /<A HREF="(2015\d{4})\/\1\.html"[^>]*>/gi;
    let match;
...
async function downloadEventDayIndexPages() {
    try {
        ...
        if (downloadCount > 0 || skippedCount > 0) {
            ...
            console.log(`\n        console.log('✅ 2015 event day index pages download complete!');`);
        } else {
