...
console.log('📸 Starting 2015 Full-Size Images Download Process...\n');
...
function mapAwardToDirectory() {
    ...
    // Read the 2015-index.html to map awards to their directories
    const indexPath = path.join(HTML_DIR, '2015-index.html');
    if (fs.existsSync(indexPath)) {
        const indexContent = fs.readFileSync(indexPath, 'utf8');
        
        // Look for patterns like: <A HREF="20150103/20152291.html">
        const linkRegex = /<A HREF="([^"]+\/(20152\d{3})\.html)"/gi;
        let match;
...
function extractAwardImages() {
    ...
    // Get all award HTML files (20152XXX.html pattern)
    const awardFiles = fs.readdirSync(HTML_DIR)
        .filter(file => file.match(/^20152\d{3}\.html$/));
    ...
    for (const awardFile of awardFiles) {
        ...
        // Look for image references like: <IMG SRC="20152291.jpg"
...
async function downloadFullSizeImages() {
    try {
        ...
        if (downloadCount > 0 || skippedCount > 0) {
            ...
            console.log(`\n✅ 2015 full-size image download complete!`);
        } else {
