...
const jsonDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2016/data/json'));
const htmlDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2016/html'));
 ...
 * @param {string} dateStr - Date string like "February 15, 2016"
 * @returns {string} - YYYYMMDD format like "20160215"
 */
...
function applyMissingInfoLogic(extractedData) {
    ...
    // Apply specific award fixes from 2016logic.txt FIRST (before general logic)
    extractedData = applySpecific2016Fixes(extractedData);
...
/**
 * Apply specific 2016 award fixes based on 2016logic.txt and orchidNameDecisionTree.json
 * 
 ...
 */
function applySpecific2016Fixes(extractedData) {
    const awardNum = extractedData.awardNum;
    
    // Apply display award logic for all display awards per 2016logic.txt
    ...
    if (isDisplayAward) {
        ...
        // Set plant fields per 2016logic.txt: "Put N/A for any species not specified"
    ...
    try {
        ...
        if (mainFont.length > 0) {
            ...
            if (lines.length > 0) {
                ...
                // Format 1: "October 4, 2016 San Francisco Monthly"
                ...
                if (firstLine.includes(' - ')) {
                    ...
                    // Look for year (2016) or month patterns
                    ...
                    if (monthDayMatch) {
                        return {
                            date: monthDayMatch[1].trim() + ', 2016', // Add year since we know it's 2016
...
function extractFullAwardDataFromHtml(awardNum) {
    ...
    try {
        ...
        const extractedData = {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2016/html/${awardNum}.html`,
            year: 2016
        };
...
function processAll2016Files() {
        console.log('🚀 PROCESSING ALL 2016 HTML FILES TO JSON (Enhanced with Logic Reference)');
    ...
    // Get all HTML files, excluding summary pages (20160xxx are summary pages, 20165xxx are individual awards)
    const htmlFiles = fs.readdirSync(htmlDirectory)
        .filter(file => file.endsWith('.html') && file !== '2016.html')
        .filter(file => !file.match(/^20160/)) // Skip summary pages like 20160120.html
    ...
    const reportPath = path.join(jsonDirectory, '../2016-enhanced-processing-report.json');
    const report = {
        timestamp: new Date().toISOString(),
 ...
 * @param {string} year - The year for the analysis (default: 2016)
 */
function saveAnalysisToFile(analysis, focusedMode, year = '2016') {
 ...
 */
async function analyze2016Data(focusedMode = true, saveToFile = true) {
    console.log(focusedMode ? '🔍 2016 AWARDS - MISSING DATA ANALYSIS' : '📊 2016 AWARDS - FULL DATA ANALYSIS');
    console.log('='.repeat(80));
    ...
    try {
        ...
        if (saveToFile) {
            saveAnalysisToFile(analysis, focusedMode, '2016');
        }
