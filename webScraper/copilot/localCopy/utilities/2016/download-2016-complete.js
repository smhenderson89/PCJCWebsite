...
const INDEX_URL = 'https://www.paccentraljc.org/2016.html';
...
function extractAwardLinks(htmlContent) {
    ...
    // Look for links that match 2016 award patterns
    ...
    while ((match = linkRegex.exec(htmlContent)) !== null) {
        ...
        // Match 2016 award number patterns (20164XXX)
        if (href.match(/20164\d{3}\.html$/)) {
            links.push({
                href: href,
...
async function downloadAwards() {
    try {
        ...
        if (awardLinks.length === 0) {
            ...
            const allLinks = [];
            const altRegex = /href=["']([^"']*(?:2016|award)[^"']*)["']/gi;
            let altMatch;
            ...
            console.log(`\n🔍 Alternative parsing found ${allLinks.length} links with "2016" or "award":`);
        ...
        if (downloadCount > 0) {
            ...
            console.log(`\n✅ 2016 download complete! You can now run the analysis script.`);
        } else {
