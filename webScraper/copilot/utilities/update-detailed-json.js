...
class DetailedJsonUpdater {
    ...
    async updateAllDetailedJsonFiles() {
        console.log('🚀 Starting Detailed JSON Files Update\n');
        console.log('📋 This will extract structured orchid award data:');
        ...
        // Get all HTML files (excluding index)
    ...
    async getHtmlFiles() {
        try {
            ...
            const htmlFiles = files
                ...
                .filter(f => f !== '2025') // Exclude index page
                .sort();
            return htmlFiles;
        } catch (error) {
    ...
    async extractDetailedAwardData(awardNumber) {
        ...
        return {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2025/html/${awardNumber}.html`,
            year: 2025
        };
    }
    ...
    extractLocation(text) {
        // Extract location from beginning of text
...
async function updateDetailedJsonFiles() {
    console.log('🚀 Starting Detailed JSON Files Update Process\n');
    
    try {
        const updater = new DetailedJsonUpdater();
        const results = await updater.updateAllDetailedJsonFiles();
        
        console.log('\n🎉 DETAILED JSON UPDATE COMPLETE!');
        ...
        console.error('❌ Error during detailed JSON update:', error);
    }
}

if (require.main === module) {
    updateDetailedJsonFiles().catch(console.error);
}

module.exports = { updateDetailedJsonFiles };