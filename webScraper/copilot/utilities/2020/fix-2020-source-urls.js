...
/**
 * Fix 2020 Source URLs
 * 
 * Updates the sourceUrl field in 2020 JSON files to reflect the actual source URL structure:
...
class SourceUrlFixer2020 {
    constructor() {
        this.baseDir = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2020/data/json');
        this.backupDir = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2020/data/backups');
        this.results = {
            processed: 0,
            updated: 0,
            errors: [],
     ...
     * @param {string} dateStr - Date string like "January 24, 2020"
     * @returns {string} - YYMMDD format like "200124"
     */
    ...
    /**
     * Process all JSON files in the 2020 directory
     */
    async processAllFiles() {
        console.log('🔧 Fixing source URLs for 2020...');
    ...
    async generateReport() {
        const report = {
            ...
            timestamp: new Date().toISOString(),
            operation: 'fix-2020-source-urls'
        };

        const reportPath = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2020/data/2020-source-url-fixes-report.json');
...
async function main() {
    try {
        const fixer = new SourceUrlFixer2020();
        await fixer.processAllFiles();
        
        console.log(`\n🎉 Source URL correction complete for 2020!`);
...
module.exports = SourceUrlFixer2020;