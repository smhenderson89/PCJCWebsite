...
const jsonDirectory = '/Users/scotthenderson/Programming/Freelancing/Orchid Society/Github/PCJCWebsite/webScraper/copilot/savedData/2023/json';
const htmlDirectory = '/Users/scotthenderson/Programming/Freelancing/Orchid Society/Github/PCJCWebsite/webScraper/copilot/localCopy/paccentraljc.org/awards/2023/html';
const indexHtmlPath = '/Users/scotthenderson/Programming/Freelancing/Orchid Society/Github/PCJCWebsite/webScraper/copilot/localCopy/paccentraljc.org/awards/2023/index.html';
const reportPath = '/Users/scotthenderson/Programming/Freelancing/Orchid Society/Github/PCJCWebsite/webScraper/copilot/savedData/2023/2023-missing-data-report.json';
...
function extractLocationFromIndexPage() {
    ...
    try {
        ...
        // Look for table rows with location information
        $('table tr').each((i, row) => {
            const $row = $(row);
            ...
            if (firstCell.length > 0) {
                ...
                // Extract location from text like "Jan 20 - Filoli Historic House Monthly"
                const locationMatch = cellText.match(/.*?-\s*(.+)$/);
                if (locationMatch) {
                    const location = locationMatch[1].trim();
                    
                    // Find all award links in the second cell
                    const secondCell = $row.find('td').eq(1);
                    secondCell.find('a').each((j, link) => {
                        const href = $(link).attr('href');
                        if (href) {
                            const awardNumMatch = href.match(/(\d{8})/);
                            if (awardNumMatch) {
                                const awardNum = awardNumMatch[1];
                                locationMap[awardNum] = location;
                            }
                        }
                    });
                }
...
function updateJsonFilesWithLocations(jsonAnalysis) {
    let updatedCount = 0;
    
    for (const analysis of jsonAnalysis) {
        if (analysis.locationConflictResolved || (analysis.location && !analysis.originalLocation)) {
            try {
                const filePath = path.join(jsonDirectory, `${analysis.awardNum}.json`);
                const content = fs.readFileSync(filePath, 'utf8');
                const data = JSON.parse(content);
                ...
                console.error(`Error updating ${analysis.awardNum}.json:`, error);
            }
        }
    }
...
function analyze2023Data() {
    console.log('Starting comprehensive analysis of 2023 data completeness...');
    ...
    console.log('Updating JSON files with corrected locations...');
    const updatedFiles = updateJsonFilesWithLocations(jsonAnalysis);
    console.log(`Updated ${updatedFiles} JSON files with location data`);
    
    // Rebuild stats after location updates
    for (const analysis of jsonAnalysis) {
    ...
    // Console summary
    console.log('\n' + '='.repeat(70));
    console.log('2023 PCJC AWARDS DATA - COMPREHENSIVE ANALYSIS REPORT');
