...
/**
 * Fix 2022 Source URLs
 * 
 * Updates the sourceUrl field in 2022 JSON files to reflect the actual source URL structure:
...
class SourceUrlFixer2022 {
    constructor() {
        this.baseDir = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2022/data/json');
        this.backupDir = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2022/data/backups');
        this.results = {
            processed: 0,
            updated: 0,
            errors: [],
     ...
     * @param {string} dateStr - Date string like "January 20, 2022"
     * @returns {string} - YYYYMMDD format like "20220120"
     */
    ...
    async fixSourceUrl(filePath) {
        try {
            ...
            // Need to fix if: contains '/awards/' OR has 6-digit date format (220301 instead of 20220301)
            const needsFixing = data.sourceUrl && (
                data.sourceUrl.includes('/awards/') ||
                /\/\d{6}\//.test(data.sourceUrl) // Matches /YYMMDD/ pattern that needs conversion to YYYYMMDD
            );
    ...
    /**
     * Process all JSON files in the 2022 directory
     */
    async processAllFiles() {
        console.log('🔧 Fixing source URLs for 2022...');
    ...
    async generateReport() {
        const report = {
            ...
            timestamp: new Date().toISOString(),
            operation: 'fix-2022-source-urls'
        };

        const reportPath = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2022/data/2022-source-url-fixes-report.json');
...
async function main() {
    try {
        const fixer = new SourceUrlFixer2022();
        await fixer.processAllFiles();
        
        console.log(`\n🎉 Source URL correction complete for 2022!`);
...
module.exports = SourceUrlFixer2022;