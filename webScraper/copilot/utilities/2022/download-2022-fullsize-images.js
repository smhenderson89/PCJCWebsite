...
class Awards2022FullImageDownloader {
    constructor() {
    ...
    async downloadFullSizeImages() {
        console.log('🚀 Starting 2022 Full-Size Image Download Process\n');
    ...
    async buildAwardToDateMapping() {
        ...
        // Read the 2022 index file to build the mapping
        const indexPath = path.join(this.htmlDir, '2022-index.html');
        if (!fs.existsSync(indexPath)) {
            throw new Error('2022 index file not found');
        }
    ...
    generateReport() {
        ...
        const reportPath = path.join(this.jsonDir, '2022-fullsize-images-download-report.json');
        ...
        console.log('\n✅ 2022 Full-size images download complete!');
    }
...
// Main execution
async function main() {
    const downloader = new Awards2022FullImageDownloader();
...
module.exports = Awards2022FullImageDownloader;