...
class Awards2021ImageDownloader {
    constructor() {
    ...
    async downloadAll2021Images() {
        console.log('🚀 Starting 2021 Awards Image Download Process\n');
        console.log('📋 This will:');
        console.log('   1. 📁 Scan all HTML files in the 2021 directory');
    ...
    async getHtmlFiles() {
        ...
        return files.filter(file => file.endsWith('.html') && file !== '2021-index.html')
    ...
    extractDateFolder(htmlFileName, imageSrc) {
        ...
        // Most images will be in date folders like 20210219, 20210224, etc.
        
        if (htmlFileName.match(/^(\d{8})/)) {
            return htmlFileName.match(/^(\d{8})/)[1];
        }

        // For award files like 202125301.html, extract the date part
        if (htmlFileName.match(/^(2021\d{4})/)) {
            const awardNum = htmlFileName.match(/^(2021\d{4})/)[1];
            ...
            if (awardNum.startsWith('20215301') || awardNum.startsWith('20215302') || 
                awardNum.startsWith('20215303') || awardNum.startsWith('20215304') || 
                awardNum.startsWith('20215305')) {
                return '20210219';
            } else if (awardNum.startsWith('20215401')) {
                return '20210224';
            }
    ...
    generateImageReport() {
        ...
        const reportPath = path.join(this.jsonDir, '2021-images-download-report.json');
        ...
        console.log('\n✅ 2021 Awards image download complete!');
    }
...
// Main execution
async function main() {
    const downloader = new Awards2021ImageDownloader();
    
    try {
        await downloader.downloadAll2021Images();
    } catch (error) {
...
module.exports = Awards2021ImageDownloader;