...
/**
 * Fix 2021 Source URLs
 * 
 * Updates the sourceUrl field in 2021 JSON files to reflect the actual source URL structure:
...
class SourceUrlFixer2021 {
    constructor() {
        this.baseDir = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2021/data/json');
        this.backupDir = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2021/data/backups');
        this.results = {
            processed: 0,
            updated: 0,
            errors: [],
     ...
     * @param {string} dateStr - Date string like "September 07, 2021"
     * @returns {string} - YYMMDD format like "210907"
     */
    ...
    /**
     * Process all JSON files in the 2021 directory
     */
    async processAllFiles() {
        console.log('🔧 Fixing source URLs for 2021...');
    ...
    async generateReport() {
        const report = {
            ...
            timestamp: new Date().toISOString(),
            operation: 'fix-2021-source-urls'
        };

        const reportPath = path.join(__dirname, '../../localCopy/paccentraljc.org/awards/2021/data/2021-source-url-fixes-report.json');
...
async function main() {
    try {
        const fixer = new SourceUrlFixer2021();
        await fixer.processAllFiles();
        
        console.log(`\n🎉 Source URL correction complete for 2021!`);
...
module.exports = SourceUrlFixer2021;