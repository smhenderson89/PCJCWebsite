/**
 * 2018 Master Data Check Script
 * Comprehensive verification of 2018 orchid awards data
...
// Path to the 2018 JSON data directory
const jsonDataPath = path.join(__dirname, '../../paccentraljc.org/awards/2018/data/json');
...
function checkIsDisplayAward(data) {
    ...
    // Check award type for special awards (show trophies, AQ awards, etc.)
    if (data.award) {
        const award = data.award.toLowerCase();
        if (award.includes("trophy") || award.includes("best") || award.includes("special") || 
            award === "aq" || award.includes("award of quality")) {
            return true;
        }
    }
    ...
    if (data.species) {
        ...
        if (species.includes("display")) {
            return true;
        }
    }
    
    // Check description for display indicators
...
function runMasterCheck() {
    console.log('🔍 2018 MASTER DATA CHECK');
    ...
    const reportPath = path.join(__dirname, '2018-master-check-report.json');
...
function printSummary(results) {
    console.log('\n' + '=' .repeat(80));
    console.log('📊 2018 DATA VERIFICATION SUMMARY');
    ...
    if (results.filesWithIssues === 0) {
        console.log('🎉 ALL DATA CHECKS PASSED! The 2018 dataset is clean and properly categorized.');
    } else {
