...
const jsonDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2018/data/json'));
const htmlDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2018/html'));
 ...
 * @param {string} dateStr - Date string like "February 15, 2018"
 * @returns {string} - YYYYMMDD format like "20180215"
 */
...
function applyMissingInfoLogic(extractedData) {
    ...
    // Apply specific award fixes from 2018logic.txt FIRST (before general logic)
    extractedData = applySpecific2018Fixes(extractedData);
...
/**
 * Apply specific 2018 award fixes based on 2018logic.txt and orchidNameDecisionTree.json
 * 
 ...
 */
function applySpecific2018Fixes(extractedData) {
    const awardNum = extractedData.awardNum;
    
    // Apply display award logic for all display awards per 2018logic.txt
    ...
    if (isDisplayAward) {
        ...
        // Set plant fields per 2018logic.txt: "Put N/A for any species not specified"
    ...
    try {
        ...
        if (mainFont.length > 0) {
            ...
            if (lines.length > 0) {
                ...
                // Format 1: "October 4, 2018 San Francisco Monthly"
                ...
                if (firstLine.includes(' - ')) {
                    ...
                    // Look for year (2018) or month patterns
                    ...
                    if (monthDayMatch) {
                        return {
                            date: monthDayMatch[1].trim() + ', 2018', // Add year since we know it's 2018
...
function extractFullAwardDataFromHtml(awardNum) {
    ...
    try {
        ...
        const extractedData = {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2018/html/${awardNum}.html`,
            year: 2018
        };
...
function processAll2018Files() {
        console.log('🚀 PROCESSING ALL 2018 HTML FILES TO JSON (Enhanced with Logic Reference)');
    ...
    // Get all HTML files, excluding summary pages (20180xxx are summary pages, 20185xxx are individual awards)
    const htmlFiles = fs.readdirSync(htmlDirectory)
        .filter(file => file.endsWith('.html') && file !== '2018.html')
        .filter(file => !file.match(/^20180/)) // Skip summary pages like 20180120.html
    ...
    const reportPath = path.join(jsonDirectory, '../2018-enhanced-processing-report.json');
    const report = {
        timestamp: new Date().toISOString(),
 ...
 * @param {string} year - The year for the analysis (default: 2018)
 */
function saveAnalysisToFile(analysis, focusedMode, year = '2018') {
 ...
 */
async function analyze2018Data(focusedMode = true, saveToFile = true) {
    console.log(focusedMode ? '🔍 2018 AWARDS - MISSING DATA ANALYSIS' : '📊 2018 AWARDS - FULL DATA ANALYSIS');
    console.log('='.repeat(80));
    ...
    try {
        ...
        if (saveToFile) {
            saveAnalysisToFile(analysis, focusedMode, '2018');
        }
