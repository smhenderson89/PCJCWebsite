 ...
 * @param {string} dateStr - Date string like "February 15, 2020"
 * @returns {string} - YYYYMMDD format like "20200215"
 */
...
function applyMissingInfoLogic(extractedData) {
    ...
    // Apply specific award fixes from 2020logic.txt FIRST (before general logic)
    extractedData = applySpecific2020Fixes(extractedData);
...
/**
 * Apply specific 2020 award fixes based on 2020logic.txt and orchidNameDecisionTree.json
 * 
 ...
 */
function applySpecific2020Fixes(extractedData) {
    const awardNum = extractedData.awardNum;
    
    // Apply display award logic for all display awards per 2020logic.txt
    ...
    if (isDisplayAward) {
        ...
        // Set plant fields per 2020logic.txt: "Put N/A for any species not specified"
    ...
    try {
        ...
        if (mainFont.length > 0) {
            ...
            if (lines.length > 0) {
                ...
                // Format 1: "October 4, 2020 San Francisco Monthly"
                ...
                if (firstLine.includes(' - ')) {
                    ...
                    // Look for year (2020) or month patterns
                    ...
                    if (monthDayMatch) {
                        return {
                            date: monthDayMatch[1].trim() + ', 2020', // Add year since we know it's 2020
...
function extractFullAwardDataFromHtml(awardNum) {
    ...
    try {
        ...
        const extractedData = {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2020/html/${awardNum}.html`,
            year: 2020
        };
...
function processAll2020Files() {
        console.log('🚀 PROCESSING ALL 2020 HTML FILES TO JSON (Enhanced with Logic Reference)');
    ...
    // Get all HTML files, excluding summary pages (20200xxx are summary pages, 20205xxx are individual awards)
    const htmlFiles = fs.readdirSync(htmlDirectory)
        .filter(file => file.endsWith('.html') && file !== '2020.html')
        .filter(file => !file.match(/^20200/)) // Skip summary pages like 20200120.html
    ...
    const reportPath = path.join(jsonDirectory, '../2020-enhanced-processing-report.json');
    const report = {
        timestamp: new Date().toISOString(),
 ...
 * @param {string} year - The year for the analysis (default: 2020)
 */
function saveAnalysisToFile(analysis, focusedMode, year = '2020') {
 ...
 */
async function analyze2020Data(focusedMode = true, saveToFile = true) {
    console.log(focusedMode ? '🔍 2020 AWARDS - MISSING DATA ANALYSIS' : '📊 2020 AWARDS - FULL DATA ANALYSIS');
    console.log('='.repeat(80));
    ...
    try {
        ...
        if (saveToFile) {
            saveAnalysisToFile(analysis, focusedMode, '2020');
        }
