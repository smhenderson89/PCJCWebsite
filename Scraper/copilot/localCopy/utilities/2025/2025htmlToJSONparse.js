...
const jsonDirectory = path.join(__dirname, '../../paccentraljc.org/awards/2025/data/json');
const htmlDirectory = path.join(__dirname, '../../paccentraljc.org/awards/2025/html');
...
function extractFullAwardDataFromHtml(awardNum) {
    ...
    try {
        ...
        const extractedData = {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2025/html/${awardNum}.html`,
            year: 2025
        };
...
function processAll2025Files() {
    console.log('🚀 PROCESSING ALL 2025 HTML FILES TO JSON (Enhanced with Logic Reference)');
    ...
    const htmlFiles = fs.readdirSync(htmlDirectory)
        .filter(file => file.endsWith('.html') && file !== '2025.html')
        .sort();
    ...
    const reportPath = path.join(jsonDirectory, '../2025-enhanced-processing-report.json');
    const report = {
        timestamp: new Date().toISOString(),
