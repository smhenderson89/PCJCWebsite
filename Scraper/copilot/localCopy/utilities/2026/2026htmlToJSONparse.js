...
const jsonDirectory = path.join(__dirname, '../../paccentraljc.org/awards/2026/data/json');
const htmlDirectory = path.join(__dirname, '../../paccentraljc.org/awards/2026/html');
const logicPath = path.join(__dirname, '../logicReference/missingInfoLogic.json');
 ...
 * @param {string} dateStr - Date string like "February 15, 2026"
...
function applyMissingInfoLogic(extractedData) {
    ...
    extractedData.logicApplied = {
        ...
        awardCategory: isDisplayAward ? 'display' : (isSpecialAward ? 'special' : (isNoPointAward ? 'no-point' : 'point-based'))
    };
...
function extractFullAwardDataFromHtml(awardNum) {
    ...
    try {
        ...
        const extractedData = {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2026/html/${awardNum}.html`,
            year: 2026
        };
        ...
        if (mainFont.length > 0) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                // Award and points (separate) - expanded to include CCE, ST, SC
                const awardMatch = line.match(/^(AM|HCC|CCM|CCE|FCC|AQ|CBR|JC|AD|CHM|ST|SC)\s+(\d+)$/i);
                if (awardMatch) {
...
function processAll2026Files() {
    console.log('🚀 PROCESSING ALL 2026 HTML FILES TO JSON (Enhanced with Logic Reference)');
    ...
    const htmlFiles = fs.readdirSync(htmlDirectory)
        .filter(file => file.endsWith('.html') && file !== '2026.html')
        .sort();
    ...
    const reportPath = path.join(jsonDirectory, '../2026-enhanced-processing-report.json');
    const report = {
        timestamp: new Date().toISOString(),
