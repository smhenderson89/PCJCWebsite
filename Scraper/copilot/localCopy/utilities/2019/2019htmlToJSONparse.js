...
const jsonDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2019/data/json'));
const htmlDirectory = path.resolve(path.join(__dirname, '../../paccentraljc.org/awards/2019/html'));
 ...
 * @param {string} dateStr - Date string like "February 15, 2019"
 * @returns {string} - YYYYMMDD format like "20190215"
 */
...
function applyMissingInfoLogic(extractedData) {
    ...
    // Apply specific award fixes from 2019logic.txt FIRST (before general logic)
    extractedData = applySpecific2019Fixes(extractedData);
...
/**
 * Apply specific 2019 award fixes based on 2019logic.txt and orchidNameDecisionTree.json
 * 
 ...
 */
function applySpecific2019Fixes(extractedData) {
    const awardNum = extractedData.awardNum;
    
    // Apply display award logic for all display awards per 2019logic.txt
    ...
    if (isDisplayAward) {
        ...
        // Set plant fields per 2019logic.txt: "Put N/A for any species not specified"
    ...
    try {
        ...
        if (mainFont.length > 0) {
            ...
            if (lines.length > 0) {
                ...
                // Format 1: "October 4, 2019 San Francisco Monthly"
                ...
                if (firstLine.includes(' - ')) {
                    ...
                    // Look for year (2019) or month patterns
                    ...
                    if (monthDayMatch) {
                        return {
                            date: monthDayMatch[1].trim() + ', 2019', // Add year since we know it's 2019
...
function extractFullAwardDataFromHtml(awardNum) {
    ...
    try {
        ...
        const extractedData = {
            ...
            htmlReference: `localCopy/paccentraljc.org/awards/2019/html/${awardNum}.html`,
            year: 2019
        };
...
function processAll2019Files() {
        console.log('🚀 PROCESSING ALL 2019 HTML FILES TO JSON (Enhanced with Logic Reference)');
    ...
    // Get all HTML files, excluding summary pages (20190xxx are summary pages, 20195xxx are individual awards)
    const htmlFiles = fs.readdirSync(htmlDirectory)
        .filter(file => file.endsWith('.html') && file !== '2019.html')
        .filter(file => !file.match(/^20190/)) // Skip summary pages like 20190120.html
    ...
    const reportPath = path.join(jsonDirectory, '../2019-enhanced-processing-report.json');
    const report = {
        timestamp: new Date().toISOString(),
 ...
 * @param {string} year - The year for the analysis (default: 2019)
 */
function saveAnalysisToFile(analysis, focusedMode, year = '2019') {
 ...
 */
async function analyze2019Data(focusedMode = true, saveToFile = true) {
    console.log(focusedMode ? '🔍 2019 AWARDS - MISSING DATA ANALYSIS' : '📊 2019 AWARDS - FULL DATA ANALYSIS');
    console.log('='.repeat(80));
    ...
    try {
        ...
        if (saveToFile) {
            saveAnalysisToFile(analysis, focusedMode, '2019');
        }
